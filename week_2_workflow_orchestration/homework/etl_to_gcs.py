...
@task(name='downloads data from url for homework', log_prints=True, retries=3,) #cache_key_fn=task_input_hash, cache_expiration=timedelta(days=1))
...
@flow(name='actual runner', log_prints=True)
def actual_runner(
        month: int, 
        year: int, 
        color: str
...
if __name__=='__main__':
    ...
    actual_runner(month,year,color)