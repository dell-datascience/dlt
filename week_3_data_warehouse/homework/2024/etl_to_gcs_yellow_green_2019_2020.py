...
def extract(data_url : str, color: str)-> pd.DataFrame:
    ...
    #                     "total_amount": "float64"
    #                     }
    df: pd.DataFrame = pd.read_parquet(data_url).astype(schema_dict, errors="ignore") #,parse_dates=[["tpep_pickup_datetime"],"tpep_dropoff_datetime"])
    return df
...
def main_flow(month: int, year: int, color: str) ->None:
    ...
    data_url = f'https://d37ci6vzurychx.cloudfront.net/trip-data/{data_file}.parquet'

    os.makedirs(Path(f"new_data/{color}/"), exist_ok=True)
    path = Path(f"new_data/{data_file}.parquet")   
